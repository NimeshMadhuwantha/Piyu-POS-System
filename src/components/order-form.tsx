...
import { calculateItemsWeight, calculateLineSubtotal, calculateTotals, formatLKR } from "@/lib/calculations";
...
export function OrderForm({ existing }: { existing?: Order }) {
  ...
  const defaults: OrderFormValues = existing ? { orderId: defaultOrderId, customer: existing.customer, items: editableItems(existing), shipping: existing.shipping, payment: existing.payment, orderDiscount: existing.orderDiscount, deliveryCharge: existing.deliveryCharge, amountPaid: existing.amountPaid, notes: existing.notes || "", requestDate: existing.requestDate, deliveryDate: existing.deliveryDate || "" } : { orderId: defaultOrderId, customer: { name: "", email: "", mobile1: "", mobile2: "", address1: "", address2: "", city: "", district: "", note: "" }, items: [newItem()], shipping: { method: "", methodId: "", courier: "", trackingNumber: "", parcelWeight: 0, note: "" }, payment: { method: "Cash on Delivery (COD)", status: "Pending" }, orderDiscount: 0, deliveryCharge: 0, amountPaid: 0, notes: "", requestDate: today(), deliveryDate: "" };
