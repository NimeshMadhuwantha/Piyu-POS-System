  ...
  return <div className="dispatch-label">
    ...
    <div className="dispatch-columns">
      <section className="dispatch-sender">
        ...
        <div className="dispatch-values"><span><b>Weight</b>{delivery ? `${delivery.parcelWeight.toLocaleString("en-LK", { maximumFractionDigits: 2 })} g` : "-"}</span><span><b>Value</b>{delivery ? formatLKR(delivery.value) : "-"}</span>{delivery?.deliveryPaid && <span className="dispatch-paid"><b>Payment</b>Deliver Paid</span>}</div>
      </section>
      <section className="dispatch-recipient"><small>DELIVER TO</small><h2>{order.customer.name}</h2>{address && <p className="dispatch-address">{address}</p>}{mobileNumbers && <p><b>Mobile:</b> {mobileNumbers}</p>}{hasText(order.customer.email) && <p><b>Email:</b> {order.customer.email}</p>}{hasText(order.shipping.trackingNumber) && <p><b>Tracking:</b> {order.shipping.trackingNumber}</p>}{hasText(order.shipping.method) && <p><b>Shipping:</b> {order.shipping.method}{hasText(order.shipping.courier) && ` · ${order.shipping.courier}`}</p>}{hasText(order.customer.note) && <p><b>Client note:</b> {order.customer.note}</p>}{hasText(order.shipping.note) && <p><b>Shipping note:</b> {order.shipping.note}</p>}</section>
    </div>
