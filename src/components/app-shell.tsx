...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div>
    <aside className="app-sidebar"><Link className="brand brand-home" href="/dashboard" aria-label="Go to Dashboard"><Image className="brand-logo" src="/icons/piyu%20logo.png" alt="Piyu logo" width={48} height={40} priority/><div>Piyu POS<small>Order Management</small></div></Link>{navigation()}</aside>
    ...
    <div className={`mobile-menu-layer ${menuOpen ? "open" : ""}`} aria-hidden={!menuOpen}>
      ...
      <aside id="mobile-drawer" className="mobile-drawer" aria-label="Mobile menu">
        <div className="drawer-head"><Link className="brand brand-home" href="/dashboard" aria-label="Go to Dashboard" onClick={() => setMenuOpen(false)}><Image className="brand-logo" src="/icons/piyu%20logo.png" alt="Piyu logo" width={48} height={40}/><div>Piyu POS<small>Order Management</small></div></Link><button type="button" aria-label="Close menu" onClick={() => setMenuOpen(false)}><X size={24}/></button></div>
        {navigation(true)}
      </aside>
    </div>
