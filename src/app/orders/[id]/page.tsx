...
const statuses: OrderStatus[] = ["Pending", "Confirmed", "Processing", "Packed", "Shipped", "Delivered", "Delivery Failed", "Cancelled", "Returned"];
...
export default function OrderDetail() {
  ...
  function changeStatus(status: OrderStatus) {
    ...
    if (["Cancelled", "Returned"].includes(status) && !confirm(`Mark this order as ${status}?`)) return;
  ...
  return <>
    <div className="no-print">
      <div className="page-head"><div><h1>{order.orderCode}</h1><span className="muted">{order.pending ? "Waiting to sync" : "Synced"}</span></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Link className="btn secondary" href={`/orders/${id}/edit`}><Edit3 size={17}/>Edit</Link><button className="btn" onClick={() => print("full")}><Printer size={17}/>Print full bill</button>{user?.role === "admin" && <button className="btn danger" onClick={() => setDeleteOpen(true)}><Trash2 size={17}/>Delete</button>}</div></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:14}}>
        <section className="card"><h2 className="section-title">Customer details</h2><b>{order.customer.name}</b><p>{order.customer.mobile1}{order.customer.mobile2 && ` / ${order.customer.mobile2}`}</p><p>{[order.customer.address1,order.customer.address2,order.customer.city,order.customer.district].filter(Boolean).join(", ")}</p>{order.customer.note && <p className="muted">{order.customer.note}</p>}</section>
        ...
        <section className="card"><h2 className="section-title">Payment & status</h2><p><StatusBadge value={order.orderStatus}/> <StatusBadge value={order.paymentStatus}/></p><p>{order.payment.method}</p><p>Grand total: <b>{formatLKR(order.grandTotal)}</b><br/>Paid: {formatLKR(order.amountPaid)}<br/>Balance / COD: <b>{formatLKR(order.balance)}</b></p></section>
      </div>
      <section className="card" style={{marginTop:14}}><h2 className="section-title">Order items</h2><div className="table-wrap"><table className="table"><thead><tr><th>Item</th><th>Variant</th><th>Qty</th><th>Price</th><th>Discount</th><th>Total</th></tr></thead><tbody>{order.items.map(item => <tr key={item.id}><td>{item.name}</td><td>{item.variant || "-"}</td><td>{item.quantity} {item.unit}</td><td>{formatLKR(item.unitPrice)}</td><td>{formatLKR(item.discount)}</td><td><b>{formatLKR(item.subtotal)}</b></td></tr>)}</tbody></table></div></section>
      <section className="card" style={{marginTop:14}}><h2 className="section-title">Update order status</h2><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{statuses.filter(status => status !== order.orderStatus).map(status => <button className={`btn ${["Cancelled","Returned"].includes(status) ? "danger" : "secondary"}`} key={status} onClick={() => changeStatus(status)}>Mark {status}</button>)}</div></section>
      <section className="card" style={{marginTop:14}}><h2 className="section-title">Print order</h2><p className="muted">Choose exactly which part you want to send to the printer.</p><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button className="btn secondary" onClick={() => print("shipping")}>Delivery details</button><button className="btn secondary" onClick={() => print("customer")}>Customer details & items</button><button className="btn secondary" onClick={() => print("full")}>Full bill</button><button className="btn" onClick={() => print("all")}>Print all three</button></div></section>
    </div>
