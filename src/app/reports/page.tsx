...
import { useLogs, useOrders } from "@/hooks/use-data";
import { downloadBlob, ordersCsv, reportPdf } from "@/lib/export";
import { OrderList } from "@/components/order-list";
import { primaryOrderStatus } from "@/lib/order-status";
import { ORDER_PREFIXES, ORDER_SUFFIXES } from "@/lib/order-code";
import { clearPrintTarget, openPrintDialog, setPrintTarget } from "@/lib/printing";
import { buildReportStatusDates, matchesReportFilters, reportEffectiveDate, type ReportPeriod } from "@/lib/report-filters";
...
export default function Reports() {
  ...
  const [status, setStatus] = useState<"All" | (typeof REPORT_STATUSES)[number]>("All");
  ...
  const statusDates = useMemo(() => buildReportStatusDates(logs), [logs]);
  const effectiveDates = useMemo(() => Object.fromEntries(orders.map(order => [order.id, reportEffectiveDate(order, status, statusDates)])), [orders, status, statusDates]);
  const data = useMemo(() => orders.filter(order => matchesReportFilters(order, {
    period, start, end, orderPrefix, orderSuffix, status,
  }, effectiveDates[order.id])), [orders, period, start, end, orderPrefix, orderSuffix, status, effectiveDates]);
  ...
  async function createPdf() {
    ...
    setPdfError("");
    try {
      await reportPdf(data, `Piyu POS - ${reportPeriod.label}`, `${reportPeriod.filename}.pdf`, status, effectiveDates);
    } catch (error) {
  ...
  return <main className="reports-page">
    ...
    <div className="card no-print report-filters">
      <label className="field">Date range<select value={period} onChange={event => setPeriod(event.target.value as ReportPeriod)}>{["Today", "Yesterday", "This Week", "This Month", "Custom", "All time"].map(value => <option key={value}>{value}</option>)}</select></label>
      ...
      <label className="field">Status<select value={status} onChange={event => setStatus(event.target.value as typeof status)}><option>All</option>{REPORT_STATUSES.map(value => <option key={value}>{value}</option>)}</select></label>
    ...
    {status !== "All" && <p className="muted no-print report-status-date-note">Date filters use the latest date each order changed to <b>{status}</b>.</p>}
    ...
    <OrderList key={`${period}-${start}-${end}-${orderPrefix}-${orderSuffix}-${status}`} orders={data} showWeight viewOnlyActions displayDateByOrderId={status === "All" ? undefined : effectiveDates} displayDateLabel={status === "All" ? undefined : `${status} status`}/>
  </main>;
}
