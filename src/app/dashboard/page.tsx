...
export default function Dashboard() {
  ...
  return <>
    ...
    <div className="card"><div className="page-head"><h2 className="section-title">Recent Orders</h2><Link href="/orders">View all</Link></div>{orders.length === 0 ? <div className="muted">No orders yet. Create the first order to get started.</div> : <><div className="table-wrap dashboard-orders-table"><table className="table"><thead><tr><th>Order</th><th>Date</th><th>Customer</th><th>Total</th><th>Status</th><th>Sync</th><th>Print</th></tr></thead><tbody>{orders.slice(0, 8).map(order => <tr key={order.id}><td><Link href={`/orders/${order.id}`}>{order.orderCode}</Link></td><td>{format(new Date(order.createdAtClient), "dd MMM, h:mm a")}</td><td>{order.customer.name}</td><td>{formatLKR(order.grandTotal)}</td><td><StatusBadge value={order.orderStatus}/></td><td>{order.pending ? <span style={{ color: "#b45309" }}>Waiting</span> : "Synced"}</td><td>{primaryOrderStatus(order.orderStatus) === "Pending" && <Link className="btn secondary dashboard-print" href={`/orders/${order.id}?print=item-list`}><Printer size={15}/>Item list</Link>}</td></tr>)}</tbody></table></div><div className="dashboard-orders-mobile">{orders.slice(0, 8).map(order => <article key={order.id} className="dashboard-order-card"><div><Link href={`/orders/${order.id}`}><b>{order.orderCode}</b></Link><StatusBadge value={order.orderStatus}/></div><strong>{order.customer.name}</strong><span className="muted">{format(new Date(order.createdAtClient), "dd MMM, h:mm a")}</span><b>{formatLKR(order.grandTotal)}</b><div className="dashboard-order-actions"><Link className="btn secondary dashboard-print" href={`/orders/${order.id}`}><Eye size={15}/>View bill</Link>{primaryOrderStatus(order.orderStatus) === "Pending" && <Link className="btn secondary dashboard-print" href={`/orders/${order.id}?print=item-list`}><Printer size={15}/>Item list</Link>}</div></article>)}</div></>}</div>
  </>;
}
