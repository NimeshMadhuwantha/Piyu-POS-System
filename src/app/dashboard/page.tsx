...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { label: "Today's Sales", value: formatLKR(today.filter(order => !["Cancelled", "Returned"].includes(order.orderStatus)).reduce((sum, order) => sum + order.grandTotal, 0)) },
    { label: "Pending", value: orders.filter(order => ["New", "Pending", "Confirmed"].includes(order.orderStatus)).length },
    { label: "Processing", value: orders.filter(order => order.orderStatus === "Processing").length },
    { label: "Shipped", value: orders.filter(order => order.orderStatus === "Shipped").length },
    ...
    { label: "Returned", value: orders.filter(order => order.orderStatus === "Returned").length },
  ];
  return <>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 12, marginBottom: 20 }}>{stats.map(stat => <div className="card" key={stat.label}><div className="muted" style={{ fontSize: 12, fontWeight: 700 }}>{stat.label}</div><div style={{ fontSize: 23, fontWeight: 800, marginTop: 9 }}>{loading ? "-" : stat.value}</div></div>)}</div>
