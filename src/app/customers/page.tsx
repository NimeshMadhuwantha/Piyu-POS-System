...
export default function Customers() {
  ...
  return <>
    ...
    <label className="field card" style={{ marginBottom: 14 }}>Search by name or mobile<input value={query} onChange={event => setQuery(event.target.value)} placeholder="Start typing…"/></label>
    <div className="card table-wrap"><table className="table"><thead><tr><th>Customer</th><th>Mobile numbers</th><th>Address</th><th>Last order</th></tr></thead><tbody>{filtered.map(customer => <tr key={customer.id}><td><Link href={`/customers/${customer.id}`}><b>{customer.name}</b></Link></td><td>{customer.mobile1}<br/>{customer.mobile2}</td><td>{[customer.address1, customer.address2, customer.city, customer.district].filter(Boolean).join(", ")}</td><td>{customer.lastOrderAt ? new Date(customer.lastOrderAt).toLocaleDateString() : "—"}</td></tr>)}</tbody></table>{!filtered.length && <p className="muted">No customers found.</p>}</div>
  </>;
}
