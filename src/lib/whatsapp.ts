...
export function deliveredInvoiceMessage(order: Order, delivery: DeliveryConfirmation) {
  ...
  return [
    ...
    items,
    "",
    delivery.trackingNumber ? `▶️ Tracking number: ${delivery.trackingNumber}` : "",
    `▶️ Order No : ${order.orderCode}`,
    `▶️ Weight: ${delivery.parcelWeight.toLocaleString("en-LK", { maximumFractionDigits: 2 })} g`,
    `▶️ Value: ${formatLKR(delivery.value)}`,
    delivery.deliveryPaid ? "▶️ Deliver Paid" : "",
    "",
