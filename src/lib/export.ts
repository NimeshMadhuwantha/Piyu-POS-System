...
import { calculateLineWeight, formatItemDiscount, orderTotalWeight } from "@/lib/calculations";
...
export function ordersCsv(orders: Order[]) {
  ...
  const rows = [[
    "No", "Created date/time", "Updated date/time", "Order ID", "Internal record ID", "Customer ID", "Customer name", "Email", "Primary mobile", "Secondary mobile", "WhatsApp number",
    ...
    order.customer.mobile1, order.customer.mobile2 || "", order.customer.whatsappNumber || "", order.customer.address1, order.customer.address2 || "", order.customer.city || "", order.customer.district || "",
    ...
    itemValues(order, item => item.unitPrice), itemValues(order, formatItemDiscount), itemValues(order, item => item.subtotal),
